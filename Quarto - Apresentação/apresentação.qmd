...
p1 <- ggplot(db, aes(x = quant_artigos)) +
  ...
  annotate("text", x = 40, y = 0.058, label = "Média população\nde Pesquisa: 28.10", size = 8, color = "black", lineheight = 0.5) +
...
p2 <- ggplot(df_plot, aes(x = Amostra, y = Media)) +
  ...
  theme(panel.grid = element_blank(),
        axis.text.y = element_blank(),
        axis.text.x = element_blank()) +
  annotate("text", x = 14, y = 54, label = "Nossa amostra tem\nMédia de 16.1", size = 8, color = "black", lineheight = 0.5) +
  annotate("curve", x = 14, xend = 15, y = 49, yend = 38, 
           curvature = 0.2, 
           arrow = arrow(type = "closed", length = unit(0.09, "inches")), 
           color = "black")
